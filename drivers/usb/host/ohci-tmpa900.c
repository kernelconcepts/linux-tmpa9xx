...
static void tmpa9x0_sram_print_stats(void)
{
	unsigned long flags;
	int i;
	void *adr;
...
static int tmpa9x0_sram_init(void *virt, void *phys)
{
	g_virt = virt;
	g_phys = phys;
...
void *tmpa9x0_sram_alloc(int o_size)
{
	unsigned long flags;
...
static int ohci_hcd_tmpa900_drv_probe(struct platform_device *pdev)
{
	...
	tmpa9x0_sram_init(sram_virt, (void *)mem->start);
	...
	hcd = usb_create_hcd(driver, &pdev->dev, "tmpa9x0");
	if (!hcd) {
		retval = -ENOMEM;
		goto err2;
	}

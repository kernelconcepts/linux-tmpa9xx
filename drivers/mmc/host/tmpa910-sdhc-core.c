/*
 * drivers/mmc/host/tmpa910-sdhc-core.c
 *
 * Copyright (C) 
 *
...
//#include <mach/dmac_regs.h>

#include "tmpa910-sdhc.h"
...
#define DRIVER_NAME	"tmpa910-sdhc"
...
struct sdhc_host {
	...
	dma_addr_t sg_dma;
	struct tmpa910_dma_desc *sg_cpu;
...
static int prepare_data_dma(struct sdhc_host *host)
{
	...
	for_each_sg(data->sg, sg, data->sg_len, i) {
		...
		lli->dma_lli = host->sg_dma + (i + 1) * sizeof(struct tmpa910_dma_desc);
	}
...
#define MIN_CLOCK_DIVIDER 	2
	...
	host->dma_channel = tmpa910_dma_request("TMPA9xx SDHC", 0, dma_interrupt, NULL, host);
	if (host->dma_channel < 0) {
		printk(KERN_ERR "tmpa910_dma_request() failed\n");
		ret = -ENOMEM;
		goto err2;
	}
...
err3:
	tmpa910_dma_free(host->dma_channel);
err2:
	sdhc_deinit();
err1:
	mmc_free_host(mmc);

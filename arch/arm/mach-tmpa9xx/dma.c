 ...
 *
 *  tmpa9xx DMA registration and IRQ dispatching
...
/**
 * tmpa9xx_dma_enable - function to start TMPA9XX DMA channel operation
 */
void tmpa9xx_dma_enable(int dma_ch)
{
	...
	pr_debug("tmpa9xx dma%d: tmpa9xx_dma_enable\n", dma_ch);
...
/**
 * tmpa9xx_dma_disable - disable TMPA9XX DMA channel operatin
 */
...
/**
 * tmpa9xx_dma_request - request/allocate specified channel number
 */
		...
		void *data)
{
	...
	for (i = prio; i > 0; i--) {
		if (tmpa9xx_dma_channels[i].name == NULL) {
			found = 1;
			break;
		}
	}
	...
	if (!found) {
		for (i = prio; i < TMPA9XX_DMA_CHANNELS; i ++) {
			if (tmpa9xx_dma_channels[i].name == NULL) {
				found = 1;
				break;
			}
	...
	tmpa9xx_dma_channels[i].name = name;
	tmpa9xx_dma_channels[i].err_handler = err_handler;
	tmpa9xx_dma_channels[i].irq_handler = irq_handler;
	tmpa9xx_dma_channels[i].data = data;
...
/**
 * tmpa9xx_dma_free - release previously acquired channel
 */
...
{
	struct tmpa9xx_dma_channel *channel;
...
static int __init tmpa9xx_dma_init(void)
{
	...
	for (i = 0; i < TMPA9XX_DMA_CHANNELS; i++) {
		tmpa9xx_dma_channels[i].name = NULL;
		tmpa9xx_dma_channels[i].dma_num = i;
	}
...
EXPORT_SYMBOL(tmpa9xx_dma_get_size);
